  ...
  try {
    ...
    for (let dbRaceIdx = 0; dbRaceIdx < dbRaces.length; dbRaceIdx++) {
      ...
      try {
        ...
        db.prepare(
          'UPDATE races SET trap_fav=?,name_fav=?,trap_und=?,name_und=?,pct=?,nivel=?,perfil_fav=?,perfil_und=?,obs=?,hist_fav=?,hist_und=?,hist_all=?,race_card=?,top3=?,track_full=?,eliminados=?,post_pick=?,scores_json=? WHERE id=?'
        ).run(
          ...
          novoResultado.eliminados ? JSON.stringify(novoResultado.eliminados) : null,
          novoResultado.postPick || null,
