...
${races.filter(r=>r.nivel!=='skip'&&r.trap_fav>0).map(r=>{var bc=r.nivel==='alta'?'ba':r.nivel==='media'?'bm':'bb';var horaBr=r.hora_br||r.hora||'-';var horaUk=r.hora||'';return`<tr><td style="white-space:nowrap"><strong style="color:#22c55e;font-size:13px">${horaBr}</strong><div style="font-size:9px;color:rgba(34,197,94,.4)">${horaUk}</div></td><td><div style="font-weight:700;font-size:12px">${r.corrida||'-'}</div><div style="font-size:10px;color:#666">${r.dist||''}</div></td><td style="white-space:nowrap"><span class="trap-badge t${r.trap_fav}">${r.trap_fav}</span> vs <span class="trap-badge t${r.trap_und}">${r.trap_und}</span><br><a style="font-size:9px;color:rgba(96,165,250,.7);cursor:pointer" onclick="openSessValModal(${r.id})">&#128269; ver historico</a></td><td style="white-space:nowrap"><span class="badge ${bc}">${r.nivel}</span><div style="font-size:10px;color:#888;margin-top:2px">${r.pct}%</div></td><td style="font-size:10px;color:#888;white-space:nowrap">${r.perfil_fav||''}<br>${r.perfil_und||''}</td><td style="font-size:11px;color:#888;max-width:200px;line-height:1.4">${r.obs||'-'}</td><td style="text-align:center">${r.odd||'-'}</td><td style="text-align:center">${r.valor?'R$'+r.valor:'-'}</td><td style="font-size:11px;text-align:center">${(function(){var tc=["","t1","t2","t3","t4","t5","t6"];var html="";[r.resultado_1,r.resultado_2,r.resultado_3].forEach(function(v){if(!v)return;var n=parseInt(v);if(n>=1&&n<=6){html+='<span class="trap-badge '+tc[n]+'" style="width:20px;height:20px;font-size:10px;margin:0 1px">'+n+'</span>';}else{html+='<span style="font-size:9px;color:#888">'+String(v).split(" ")[0].slice(0,8)+'</span>';}});return html||"-";})()}</td><td style="text-align:center" class="${r.bateu==='sim'?'sim':r.bateu==='nao'?'nao':''}">${r.bateu==='sim'?'✓':r.bateu==='nao'?'✗':'-'}</td></tr>`;}).join('')}
...
function openSessValModal(id){
  ...
  if(!hf&&!hu){
    document.getElementById('sv-title').textContent='Historico indisponivel';
    document.getElementById('sv-body').innerHTML='<p style="color:#888;font-size:12px;padding:20px;text-align:center">Sessao salva antes do recurso ser ativado.</p>';
    document.getElementById('sv-modal').classList.add('open');return;
  }
...
function svCard(trap,nome,perfil,hist){
  ...
  var rows=hist.map(function(h){
    ...
    var rem=h.remarks||'';
    var ci=rem.indexOf(',');if(ci>=0){var ws=rem.lastIndexOf(' ',ci)+1;rem=rem.substring(ws);}
